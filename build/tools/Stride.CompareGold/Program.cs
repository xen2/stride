...
app.MapGet("/api/ci/runs", async (string? branch, int? limit) =>
{
    ...
    var tasks = repos.Select(async r =>
    {
        var runs = await FetchJsonLinesAsync(
            $"api repos/{r}/actions/runs --jq \".workflow_runs[:{ lim}] | .[] | {{id: .id, head_sha: .head_sha, head_branch: .head_branch, created_at: .created_at, conclusion: .conclusion, name: .name}}\" {branchArg}");
